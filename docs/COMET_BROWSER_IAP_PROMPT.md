...
【目標】
プレミアムプラン（月額1,000円（年額8,000円））のサブスクリプション商品を作成する
...
【手順3: 無料トライアル】
- **無料トライアル**: 設定しない
  (storekit 未設定、広告詐欺リスク回避のため今回はスキップ)
...
設定完了後、以下を確認：
- ✅ 商品ID: premium_monthly
- ✅ 価格: ¥1,000/月（年額: premium_yearly ¥8,000）
- ✅ 期間: 1ヶ月
- ✅ 無料トライアル: 未設定（意図的）
- ✅ 自動更新: 有効
- ✅ 審査メモ: 入力済み
...
2. 価格は後から変更可能です
3. 無料トライアルは現状設定しません（storekit と一致させるため）
...
2. 商品ID（premium_monthly / 年額 premium_yearly）が正しく設定されたか
3. 価格（¥1,000/月、年額 ¥8,000）が正しく設定されたか
4. 無料トライアルが設定されていない（意図通り）か
5. エラーメッセージがあればその内容
...
- 日本円で ¥1,000 を設定すると、他の国の価格は自動で調整されます
- 後から価格変更も可能です

### 無料トライアルについて

- 現状は設定しない（storekit と整合性を保つため）
- 将来的に検討する場合は storekit と App Store Connect の両方を更新
