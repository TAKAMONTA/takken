...
1. **App Store Connect で IAP を作成・提出**
   ...
   - 以下の **サブスクリプション** を作成し、**審査に提出** する:
     - **月額**: 商品 ID `com.takamonta.takken.premium.monthly`
     - ※年額プランは廃止しました（コードからも完全削除済み）。
...
3. **審査用メモ（Review Notes）**
   - 再提出時の **審査メモ** に、例えば次のように記載することを推奨します:
     - 「App 内課金（月額¥1,000のみ。年額プランは廃止）のプレミアムプランを提出しました。購入はプレミアムプラン画面の『月額プランに登録』から行えます。」
...
- [ ] IAP: 月額の App 内課金を作成し、App Review 用スクリーンショットを添付して審査に提出した
...
4. App 内課金の提出（2.1）
   - プレミアムプラン（月額のみ。年額プランは廃止）の App 内課金を登録し、
